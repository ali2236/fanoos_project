...
class Services extends StatefulWidget {
  ...
  const Services({Key key, @required this.container, @required this.child})
      : super(key: key);
  ...
  static Future<void> run<T extends AppService>(BuildContext context,
      {bool bind}) {
    return ScopedModel.of<T>(context, rebuildOnChange: bind)
        .run(getContainer(context));
  }
}
...
class _ServicesState extends State<Services> {
  ...
  Widget builder(Widget child, Queue<Model> models) {
    ...
    return ScopedModel(
        model: model, child: models.isEmpty ? child : builder(child, models));
  }
}
